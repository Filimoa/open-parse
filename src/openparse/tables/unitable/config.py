...
class StructureModelConfig(BaseModel):
    ...
    vocab_path: Path = root / "tables/unitable/vocab/vocab_html.json"
    max_seq_len: int = 784


class BboxModelConfig(BaseModel):
    weights_path: Path = root / "weights/unitable/unitable_large_bbox.pt"
    vocab_path: Path = root / "tables/unitable/vocab/vocab_bbox.json"
...
class ContentModelConfig(BaseModel):
    ...
    vocab_path: Path = root / "tables/unitable/vocab/vocab_cell_6k.json"
    max_seq_len: int = 200
